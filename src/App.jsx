...
export default function App() {
  ...
  var handleUpdatePanelEquipment = useCallback(function(panelId, newEqs) {
    pushUndo()
    setEquipmentMap(function(prev) {
      var next = Object.assign({}, prev)
      next[panelId] = newEqs
    ...
    // Debounced save (1.5s) handles persistence. No flush — that would
    // block the UI on every fill-drag cell. Deletes also survive via the
    ...
    <div className="min-h-screen bg-navy text-white">
      ...
      {traceSession && (
        <Suspense fallback={pageLoading()}>
          <TraceStudio file={traceSession.file} record={traceSession.record} projectId={activeProject && activeProject.id} onCancel={handleTraceCancel} onComplete={handleTraceComplete} />
        </Suspense>
      )}
      <div className="pt-14 md:pt-0 md:ml-[220px] p-4 md:p-6 print-reset">
        <Routes>
          ...
          <Route path="/panels/:panelId" element={<Suspense fallback={pageLoading()}><PanelDetail panels={panels} equipmentMap={equipmentMap} terminationMap={terminationMap} estimateScope={estimateScope} projectId={activeProject && activeProject.id} onUpdatePoint={handleUpdatePoint} onUpdateEquipment={handleUpdatePanelEquipment} onUpdateTermination={handleUpdateTermination} onDeletePanel={handleDeletePanel} onUndo={handleUndo} canUndo={canUndo} /></Suspense>} />
          <Route path="/field-devices" element={<Suspense fallback={pageLoading()}><CommDevices loops={loops} areas={areaGroups} gateways={gateways} projectId={activeProject && activeProject.id} onUpdateLoops={handleUpdateLoops} onUpdateAreas={handleUpdateAreas} onUpdateGateways={setGateways} onUndo={handleUndo} canUndo={canUndo} /></Suspense>} />
          <Route path="/drawings" element={<Suspense fallback={pageLoading()}><DrawingsPage drawings={drawings} projectId={activeProject && activeProject.id} onOpen={handleOpenDrawing} onUpdateMeta={handleUpdateDrawingMeta} onDelete={handleDeleteDrawing} /></Suspense>} />
          <Route path="/documents" element={<Suspense fallback={pageLoading()}><DocumentsPage project={activeProject} panels={panels} equipmentMap={equipmentMap} onUpdateEquipment={handleUpdatePanelEquipment} documents={documents} onUpdateDoc={updateDoc} onDeleteDoc={deleteDoc} drawingsElement={<DrawingsPage drawings={drawings} projectId={activeProject && activeProject.id} onOpen={handleOpenDrawing} onUpdateMeta={handleUpdateDrawingMeta} onDelete={handleDeleteDrawing} />} /></Suspense>} />
