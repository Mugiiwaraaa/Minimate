/* --- DesignPage.jsx --- R2 Design Engine (moved out of DocumentsPage.jsx) ---
   Design-stage workflow: import design-estimate file -> sheet picker (suggested
   pre-ticked) -> parse (estimateParser) -> confirm-first review -> SAVE to
   Estimate/Scope (data.estimateScope, the "as-designed" layer). Standalone
   cable/BOQ/containment/schedule files import the same way (kind-based).
   Split into sub-sections by sheet kind so the design data has a real home
   instead of one flat dump — DocumentsPage.jsx is now purely the file archive
   (drawings + document library), unrelated to this page. */
...
var TAB_KINDS = {
  io: ['io_summary'],
  cables: ['cables'],
  boq: ['boq', 'equipment', 'model_numbers'],
  other: ['ddc', 'analysis', 'containment', 'schedule', 'other']
}
var TAB_LABELS = { io: 'IO SUMMARY', cables: 'CABLE TAKEOFF', boq: 'BOQ / BOM', other: 'OTHER IMPORTS' }

function cableProgress(equipmentMap) {
  var total = 0, pulled = 0
  Object.keys(equipmentMap || {}).forEach(function(pid) {
    ;(equipmentMap[pid] || []).forEach(function(eq) {
      ;(eq.points || []).forEach(function(p) {
        total++
        if (p.cable_pulled) pulled++
      })
    })
  })
  return { total: total, pulled: pulled }
}
...
export default function DesignPage(props) {
  ...
  var dtabState = useState('io'); var dtab = dtabState[0]; var setDtab = dtabState[1]
  ...
  var kindsForTab = TAB_KINDS[dtab] || []
  var savedForTab = savedSheets.filter(function(s) { return kindsForTab.indexOf(s.kind) !== -1 })
  var cableProg = cableProgress(props.equipmentMap)

  return (
    <div className="p-4 md:p-6 max-w-[1200px] mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-3">
        <h1 className="text-lg md:text-xl font-bold uppercase">DESIGN <span className="text-dgray font-normal text-xs ml-2">DESIGN ENGINE — ESTIMATE, IO SUMMARY, CABLES, BOQ</span></h1>
      ...
      {busy && <div className="mb-3 text-[11px] text-teal uppercase font-semibold">{busy}</div>}

      {/* Sheet picker (spans all kinds — one import touches multiple sub-sections at once) */}
      ...
      {/* Review (all parsed sheets, regardless of active sub-tab — one combined save) */}
      ...
      {/* Saved scope for the active sub-tab */}
      <div className="bg-card rounded-xl border border-border p-4">
        <div className="text-[10px] text-dgray uppercase font-semibold mb-2">{TAB_LABELS[dtab]} (AS-DESIGNED)</div>
        {savedForTab.length === 0 ? (
          <div className="text-[11px] text-dgray uppercase">NO {TAB_LABELS[dtab]} SAVED YET — USE THE IMPORT BUTTON IN THE SIDEBAR TO ADD A DESIGN ESTIMATE.</div>
        ) : (
          <div>
      ...
      {/* Cable Takeoff sub-tab: live installed-progress rollup (cable_pulled points
          vs total, project-wide — same numbers PanelsList.jsx's getProgress() already
          computes per panel, just summed here). No per-row matching (deliberately
          simple — see plan). */}
      {dtab === 'cables' && (
        <div className="bg-card rounded-xl border border-border p-4 mt-4">
          <div className="text-[10px] text-dgray uppercase font-semibold mb-2">INSTALLED PROGRESS</div>
          {cableProg.total === 0 ? (
            <div className="text-[11px] text-dgray uppercase">NO IO POINTS ON SITE YET.</div>
          ) : (
            <div className="text-sm uppercase">
              <span className="font-extrabold text-teal">{cableProg.pulled}</span>
              <span className="text-dgray"> / {cableProg.total} POINTS CABLE PULLED </span>
              <span className="font-bold text-cyan">({Math.round(cableProg.pulled / cableProg.total * 100)}%)</span>
            </div>
          )}
        </div>
      )}

      {/* Diff vs site (IO Summary sub-tab only) — compares the saved estimate baseline
          against the LIVE as-per-site panels/equipmentMap. Works identically for a new
          build (site state built up by hand from empty) or a retrofit (site state
          seeded from an existing as-built survey import) — this just compares whatever
          the live state currently is against the design estimate, equipment-type by
          equipment-type (the I-O Summary has no panel grouping to compare against —
          see estimateDiff.js). */}
      {dtab === 'io' && estDataset && (
        <div className="bg-card rounded-xl border border-border p-4 mt-4">
